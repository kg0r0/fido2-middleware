...
import { attestationOptions, attestationResult } from "../src/attestation";
...
describe("attestationResult()", () => {
  it("should return 'Response missing one or more of id/rawId/response/type fields'", async () => {
    ...
    const requestMock = mockReq(request);
    try {
      await attestationResult(requestMock);
    } catch (e) {
      expect(e.message).to.equal(
  ...
  it("should return 'expectation did not contain value for 'challenge''", async () => {
    const request = {
      body: {
        id:
