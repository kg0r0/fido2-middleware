...
mocha_1.describe("preformatAssertionResultReq()", function () {
    ...
    mocha_1.describe("ClientDataJSONValidater()", function () {
        ...
        mocha_1.it("should return 'ok'", function () {
            ...
            var result = util_1.assertionClientDataJSONValidater(requestMock, clinetDataJSON);
            chai_1.expect(result.status).to.equal("ok");
            chai_1.expect(result.errorMessage).to.equal("");
        });
        mocha_1.it("should return 'Challenges don't match!'", function () {
            ...
            var result = util_1.assertionClientDataJSONValidater(requestMock, clinetDataJSON);
            chai_1.expect(result.status).to.equal("failed");
            chai_1.expect(result.errorMessage).to.equal("Challenges don't match!");
        });
        mocha_1.it("should return 'Origins don't match!'", function () {
            ...
            var result = util_1.assertionClientDataJSONValidater(requestMock, clinetDataJSON);
            chai_1.expect(result.status).to.equal("failed");
            chai_1.expect(result.errorMessage).to.equal("Origins don't match!");
        });
        mocha_1.it("should return 'Type don't match!'", function () {
            ...
            var result = util_1.assertionClientDataJSONValidater(requestMock, clinetDataJSON);
            chai_1.expect(result.status).to.equal("failed");
            chai_1.expect(result.errorMessage).to.equal("Type don't match!");
        });
        mocha_1.it("should return 'Token Binding don`t support!'", function () {
            ...
            var result = util_1.assertionClientDataJSONValidater(requestMock, clinetDataJSON);
            chai_1.expect(result.status).to.equal("failed");
            chai_1.expect(result.errorMessage).to.equal("Token Binding don`t support!");
        });
    });
});
